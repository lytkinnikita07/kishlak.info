...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const minimizeStickyPlayerBtn = document.getElementById("minimizeStickyPlayerBtn")
  ...
  function initAudioPlayer() {
    ...
    if (minimizeStickyPlayerBtn) {
      minimizeStickyPlayerBtn.addEventListener("click", function (e) {
        ...
        if (isMinimized) {
          ...
          stickyPlayer.style.height = "auto"
          stickyPlayer.style.width = "300px"
          ...
          stickyPlayer.style.height = "60px"
          stickyPlayer.style.width = "200px"
    ...
    // Функция для перемещения плеера
    function dragMove(e) {
  ...
  function initVideoPlayers() {
    ...
    videoWrappers.forEach((wrapper, index) => {
      ...
      // Create a thumbnail/overlay for the video
      const overlay = document.createElement("div")
      overlay.className = "video-play-overlay"
      overlay.innerHTML = '<div class="video-play-btn"><i class="fas fa-play"></i></div>'

      // Add the overlay to the wrapper
      wrapper.appendChild(overlay)

      // When overlay is clicked, load and play the video
      overlay.addEventListener("click", () => {
        // Store the original src to maintain it without adding parameters multiple times
        const originalSrc = iframe.getAttribute("data-original-src") || iframe.src
        iframe.src = originalSrc + (originalSrc.includes("?") ? "&" : "?") + "autoplay=1&controls=1&rel=0&showinfo=1"
        overlay.style.display = "none"
      })

      // Store the original src and set initial parameters to prevent autoplay
      if (!iframe.getAttribute("data-original-src")) {
        // Save the original source
        iframe.setAttribute("data-original-src", iframe.src)

        // Set initial parameters without autoplay
        iframe.src = iframe.src + (iframe.src.includes("?") ? "&" : "?") + "autoplay=0&controls=1&rel=0&showinfo=1"
      }
    })
  }
